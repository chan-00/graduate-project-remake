...
function functionUserInfo(idValue, setUserEmail, setUserComments, setUserTeamArray, setLoadingStatus, setProfileImage, setCountData) {
    //마이페이지 첫 렌더링 시 해당 유저의 이메일 값과 유저 코멘트 값을 받아오기 위한 post 코드이다.
    axios.post("http://" + process.env.server_ip + ":8000/back/mypage/", {
        id: idValue,
    }).then((res) => {
