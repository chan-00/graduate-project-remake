...
function SignUp() {
    ...
    const handleSignUp = (e) => {
        ...
        else if(emailRef.current.value.length > 30 || pwCheckRef.current.value.length === 0) {
            alert("이메일이 30글자 이상이거나 입력하지 않았습니다.");
        }
        else if(nicknameRef.current.value.length > 20 || pwCheckRef.current.value.length === 0) {
